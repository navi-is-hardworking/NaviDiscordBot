    ...
    def stop(self):
        with self.lock:
            ...
            try:
                ...
                logger.warning("Bot did not terminate gracefully, attempting force kill...")
                if sys.platform == 'win32':
                ...
                try:
                    ...
                    logger.error("Bot process could not be force killed.")
            except Exception as e:
                ...
                try:
                    ...
                    logger.error(f"Final attempt to kill bot process failed: {final_e}")
            finally:
                self.process = None
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/api/settings"):
            ...
            try:
                ...
                env_vars[token_key] = token_value
            ...
            response_data = {"running": bot_manager.is_running()}
            if success:
                response_data["success"] = True; response_data["message"] = result_msg or "Bot stopped."
                self.send_response(200)
            else:
                response_data["success"] = False; response_data["error"] = result_msg or "Failed to stop bot or bot was not running."; response_data["message"] = response_data["error"]
                self.send_response(200 if not bot_manager.is_running() and "not running" in str(result_msg).lower() else 500)
